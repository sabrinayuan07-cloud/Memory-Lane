...
// 3D circle component with GREEN theme for family
const Circle3D = ({ size, hasMemory, icon, onPress, disabled, style }) => {
  const baseColor = hasMemory ? '#5A7A5C' : '#B8D4B1';
  const darkColor = hasMemory ? '#4A6A4C' : '#A0C49D';
  const lightColor = hasMemory ? '#7A9A7C' : '#D0E4CD';
...
export default function FamilyMemoryRoad({ memories, onCirclePress }) {
  ...
  return (
    <View style={[styles.container, { height: totalHeight }]}>
      {/* SVG curved road line - GREEN theme */}
      <Svg
        width={SCREEN_WIDTH}
        height={totalHeight}
      ...
      {/* Green 3D circles with icons and labels */}
        ...
        return (
          <View key={index}>
            <Circle3D
              ...
              hasMemory={hasMemory}
              icon={icon}
              onPress={() => hasMemory && onCirclePress && onCirclePress(memory, index)}
              disabled={!hasMemory}
              style={{
